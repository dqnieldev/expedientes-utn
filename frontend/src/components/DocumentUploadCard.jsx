...
export default function DocumentUploadCard({ item, doc, icon, onUpload }) {
  ...
  const handleFileChange = (e, tipo) => {
    ...
    if (!FORMATOS_PERMITIDOS.includes(file.type)) {
      setErrorMsg(`Formato no permitido. Solo se aceptan ${FORMATOS_LABEL}.`);
      e.target.value = "";
      return;
    }
    ...
    if (file.size > TAMANO_MAX_BYTES) {
      ...
      setErrorMsg(`El archivo pesa ${sizeMB} MB. El límite es ${TAMANO_MAX_MB} MB.`);
      e.target.value = "";
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* ERROR — fuera del card para no ser cortado por overflow-hidden */}
      {errorMsg && (
        ...
        <div className="p-5">
          ...
          <div className="mb-4">
            ...
            <p className="text-[10px] text-gray-300 dark:text-gray-600 mt-1">
              {FORMATOS_LABEL} · Máx. {TAMANO_MAX_MB} MB
            </p>
          </div>
