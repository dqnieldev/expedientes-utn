...
import {
  ...
  DatabaseBackup, LogOut, ShieldCheck,
} from "lucide-react";
...
export default function AdminSidebar() {
  return (
    <>
      {/* ── DESKTOP ── */}
      <aside className="hidden md:flex w-64 flex-col bg-[#1a2744] h-full">
        ...
        <div className="px-6 pt-8 pb-6 border-b border-white/10">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-white/70 leading-tight">Administrador</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          ...
          {menu.map(({ name, path, icon: Icon, desc }) => {
            const isActive = useLocation().pathname === path ||
              (path !== "/admin/dashboard" && useLocation().pathname.startsWith(path));
            return (
              <Link key={path} to={path}
                className={`flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 group
                  ${isActive ? "bg-white/15 text-white" : "text-white/50 hover:bg-white/8 hover:text-white"}`}
              >
                <span className={`w-1 h-6 rounded-full shrink-0 ${isActive ? "bg-blue-400" : "bg-transparent"}`} />
...
function AdminBottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-[#1a2744] border-t border-white/10">
          ...
          return (
            ...
            >
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors
                ...
                <Icon size={20} className={isActive ? "text-blue-300" : "text-white/40"} />
              </div>
